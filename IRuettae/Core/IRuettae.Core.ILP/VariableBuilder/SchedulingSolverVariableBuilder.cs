...
namespace IRuettae.Preprocessing.Mapping
{
    public class SchedulingSolverVariableBuilder
    {
        ...
        public SolverInputData Build()
        {
            ...
            return new SolverInputData(santasVar, visitLength, visitsVar, distances, Visits.Select(v => v.Id).ToArray(), Santas.Select(s => s.Id).ToArray(), presolved, timeslotLength);
        }
    }
}
